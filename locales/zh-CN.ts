export default {
  leaderboard: {
    ...
    last_workout: "上次锻炼",
    page_title: "冠军排行榜",
    page_subtitle: "登上顶峰，成为 LiftTime 传奇",
  ...
  premium: {
    ...
    already_premium: "您正在享受 LiftTime Premium",
    no_ads: "无广告",
    upgrade: "升级",
    ...
    free_intro_text:
      "LiftTime 是一个免费、开源的健身应用，每天有 60,000+ 用户使用。它是由爱（不是 VC 资金 ^^）构建的，并且为我们保持运行需要真实的时间和金钱。",
    donation_story_text:
      "起初，我们依靠捐赠运行。但正如您所想，捐赠不足以支付开发和运行成本。所以我们为您制作了一个包，将帮助我们保持灯光，并在路上解锁一些超级能力。",
    health_upgrade_text: "如果 LiftTime 帮助您提升健康，请考虑升级到高级：D ！",
    ...
    still_unsure: "还不确定？没关系。LiftTime 将始终保持免费和开源。",
  ...
  release_notes: {
    ...
    notes: {
      ...
      note_2026_03_05: {
        ...
        content:
          "<li>现在可在进行中的训练会话里直接新增动作</li><li>当所有组完成后，页面不再立刻自动跳转；卡片会保留两个操作：<strong>添加一组</strong> 和 <strong>下一个动作</strong></li><li>点击进入下一个动作时，当前动作会被标记完成、自动折叠，并按完成顺序移动到列表顶部</li><li>进行中的动作会排在已完成动作之后，训练队列更清晰</li><li>会话内动作重排升级为拖拽排序</li><li>重量输入支持小数，清空输入框时不再被强制回填 0</li><li>推荐基线现在更偏向近期较重的工作组负重，减少明显偏低的预设重量</li>",
      },
      note_2026_02_20: {
        title: "📊 精准记录 + 智能训练会话控制",
        content:
          "<li>新的底部固定进度条可跟踪训练进度，并在每组后切换为休息倒计时</li><li>组和动作状态现在可一眼查看，并支持紧凑进度显示（组数 x/y）</li><li>动作卡片支持折叠，减少长训练中的滚动负担</li><li>现在可在进行中的训练会话里通过上移/下移按钮调整动作顺序</li><li>杠铃动作在重量输入下方新增清晰提示：<strong>杠铃 + 杠铃片</strong></li><li>在本地/自托管环境中，隐私偏好弹窗不再每次刷新都重复出现</li><li>品牌更新：新 Logo，并在登录、注册和统计页加入 Portland 签名文案</li><li>排行榜重置时区文案现已明确为 <strong>PST（Portland）</strong></li><li>进阶逻辑已支持训练模式（新手/进阶）扩展，跳过主观强度输入不会惩罚推荐结果</li>",
      },
      note_2026_02_19: {
        title: "💪 更智能的训练流程 + 进度追踪",
        content:
          "<li>自动填充组数现在会结合近期表现、训练目标和肌肉负荷信号</li><li>训练构建器现支持 3 种方式：器械+肌群、仅器械、或逐个选择动作</li><li>训练目标（力量、增肌、耐力）现在会影响次数、组型和默认休息时间</li><li>生成建议时会先加入热身组，再给出正式训练组</li><li>新增肌群进度面板，显示目标状态（低于目标、目标区间内、疲劳偏高）</li><li>统计新增短周期视图（1周和2周），更快获得反馈</li>",
      },
      note_2025_10_29: {
        title: "🍑 新的臀部训练计划发布！",
        content:
      ...
      note_2025_06_19: {
        title: "📱 现已作为 PWA 提供！",
        content: "LiftTime v1.2 现在是一个渐进式网络应用！将其安装在您的手机上，即可享受原生应用体验和离线访问。🚀",
      },
      note_2025_06_18: {
        title:
        ...
        content: "LiftTime 在 Hacker News 上登顶！感谢大家的大力支持，欢迎所有新用户！💪",
      },
      note_2025_06_01: {
        title: "🎉 新功能：版本说明对话框",
  ...
  donation_alert: {
    title: "使用 LiftTime Premium 解锁高级功能",
    or: "或",
  },
  ...
  commons: {
    ...
    loading: "加载中...",
    image_deleted: "图片已删除",
    discover_workoutcool: "发现 LiftTime",
