export default {
  leaderboard: {
    ...
    page_subtitle: "Поднимитесь на вершину и станьте легендой LiftTime",
    ...
    reset_timezone: "Сброс по PST (Portland)",
    reset_timezone_description: "Еженедельные и месячные рейтинги сбрасываются в полночь по PST (Portland)",
  },
  programs: {
  ...
  premium: {
    ...
    already_premium: "Вы наслаждаетесь LiftTime Premium",
    no_ads: "Без рекламы",
    upgrade: "Обновить",
    ...
    faq: {
      ...
      subtitle: "Все, что вам нужно знать о LiftTime и нашей миссии",
      items: [
        {
        ...
        {
          question: "Могу ли я самохостить LiftTime?",
          answer:
    ...
    free_intro_text:
      "LiftTime — это бесплатное приложение для фитнеса с открытым исходным кодом, которым ежедневно пользуются более 60 000 пользователей. Оно создано с любовью (а не на деньги венчурных фондов ^^) и требует реальных затрат времени и денег для поддержки.",
    donation_story_text:
    ...
    health_upgrade_text: "Если LiftTime помогает вам улучшить здоровье, пожалуйста, рассмотрите возможность перехода на Премиум :D !",
    ...
    still_unsure: "Все еще не уверены? Не волнуйтесь. LiftTime всегда останется бесплатным и с открытым исходным кодом.",
  ...
  release_notes: {
    ...
    notes: {
      ...
      note_2026_03_08: {
        ...
        content:
          "<li>Социальные ссылки в футере теперь скрываются во время активной тренировки, чтобы убрать лишние отвлечения</li><li>Мобильный layout теперь лучше использует полную высоту экрана, с более удобным нижним отступом safe-area на iOS для навигационной панели</li><li>При добавлении упражнения в середине сессии приложение теперь запрашивает рекомендации и предзаполняет подходы, если есть история</li><li>Порядок колонок в силовых подходах стабилизирован как <strong>Вес, затем Повторы</strong>, с lbs по умолчанию, чтобы избежать перестановки полей</li><li>Favicon и иконки приложения обновлены в соответствии с текущим логотипом LiftTime</li><li>Всплывающее окно donation/premium после тренировки временно отключено (кодовая основа сохранена для будущего включения)</li>",
      },
      note_2026_03_05: {
        title: "🏋️ Улучшенный поток сессии + более точные рекомендации по весу",
        content:
          "<li>Теперь можно добавлять упражнения прямо во время активной сессии</li><li>После завершения всех подходов приложение больше не переключает вас мгновенно: на карточке остаются оба действия <strong>Добавить подход</strong> и <strong>Следующее упражнение</strong></li><li>При переходе дальше завершенное упражнение отмечается как выполненное, сворачивается и переносится вверх по порядку завершения</li><li>Активные упражнения теперь идут после завершенных для более понятной очереди сессии</li><li>Перестановка упражнений в активной сессии теперь работает через drag-and-drop</li><li>Поле веса теперь поддерживает десятичные значения, а очистка поля больше не принудительно подставляет 0</li><li>База рекомендаций теперь лучше учитывает более тяжелые недавние рабочие веса, уменьшая слишком заниженные подсказки</li>",
      },
      note_2026_02_20: {
        title: "📊 Точный логинг + умное управление сессией",
        content:
          "<li>Новая закрепленная панель прогресса отслеживает тренировку и переключается на таймер отдыха после каждого подхода</li><li>Статусы подходов и упражнений теперь видны сразу, включая компактный прогресс в формате Подходы x/y</li><li>Карточки упражнений теперь можно сворачивать, чтобы сократить прокрутку в длинных сессиях</li><li>Теперь можно менять порядок упражнений в активной сессии кнопками вверх/вниз</li><li>Для упражнений со штангой под полем веса теперь есть подсказка: <strong>Штанга + блины</strong></li><li>Окно настроек приватности больше не появляется на каждом обновлении страницы в локальном/self-host окружении</li><li>Обновление брендинга: новый логотип и подпись Portland на входе, регистрации и статистике</li><li>Подпись времени сброса рейтинга теперь явно указана как <strong>PST (Portland)</strong></li><li>Логика прогрессии теперь готова к режимам (Новичок/Продвинутый), а пропуск оценки усилия не штрафует рекомендации</li>",
      },
      note_2026_02_19: {
        title: "💪 Умный тренировочный поток + отслеживание прогресса",
        content:
          "<li>Автозаполнение подходов теперь учитывает последние результаты, цель тренировки и мышечную нагрузку</li><li>Конструктор тренировок теперь поддерживает 3 режима: Оборудование + Мышцы, Только оборудование, или Индивидуальный выбор</li><li>Цели (Сила, Гипертрофия, Выносливость) теперь влияют на повторы, стиль подходов и отдых по умолчанию</li><li>Рекомендации теперь включают разминочные подходы перед рабочими</li><li>Новая панель прогресса мышц показывает статус цели (ниже цели, в цели, высокая усталость)</li><li>В статистике добавлены короткие интервалы (1 неделя и 2 недели) для более быстрого фидбека</li>",
      },
      note_2025_10_29: {
      ...
      note_2025_06_19: {
        ...
        content:
          "LiftTime v1.2 теперь является прогрессивным веб-приложением! Установите его на ваш телефон для получения нативного опыта приложения с офлайн доступом. 🚀",
      },
      note_2025_06_18: {
        title:
        ...
        content:
          "LiftTime достиг первого места на Hacker News! Спасибо всем за потрясающую поддержку и добро пожаловать всем новым пользователям! 💪",
      },
      note_2025_06_01: {
  ...
  donation_alert: {
    title: "Разблокируйте расширенные функции с LiftTime Premium",
    or: "или",
  },
  ...
  commons: {
    ...
    discover_workoutcool: "Откройте LiftTime",
